...
func (c *Container) buildParam(param reflect.Type) (val reflect.Value, err error) {
	...
	currProvider, ok := c.providers[param]
	if ! ok {
		return reflect.Value{}, fmt.Errorf("can not found provider: %s", param)
	}

	params := make([]reflect.Value, len(currProvider.params))
	for i, p := range currProvider.params {
		params[i], err = c.buildParam(p)
	}

	results := currProvider.value.Call(params)
	for _, result := range results {
		// 判断是否报错
		if isError(result.Type()) && !result.IsNil() {
			return reflect.Value{}, fmt.Errorf("%s call err: %+v", currProvider, result)
		}
